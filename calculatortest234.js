    ...
    const onDataLoaded = (data) => {
        ...
        function handleFurniture(furniture, FurnitureMarkup) {
            ...
            let showPrice = (element.shelves * 2 + element.bed + element.matress + element.kitchenCeilingLight * 2 + element.bedsideLight * 2 + element.chandelier + element.coffeeTable * (amountOfRooms - 1) + (element.cornice + element.tulle + element.curtains) * amountOfRooms) * 1.03 * (1 + (parseData("S157") / 100));
            ...
            $("#furnitureBool").siblings(".label").html(`Так <span class=\"grey\">+${returnRoundedPrice((furnitureTotal * 1.03 * (1 + (parseData("S157") / 100))) - showPrice)}$/м²</span>`);
