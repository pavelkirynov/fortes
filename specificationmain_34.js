        const onDataLoaded = (data) => {
            ...
            let $work = $("#workList"),
                $workLast = $("#workList .list-option-container").last(),
                $furniture = $("#furnitureList"),
                $furnitureLast = $("#furnitureList .list-option-container").last(),
                $materials = $("#materialsList"),
                $materialsLast = $("#materialsList .list-option-container").last(),
                $appliances = $("#appliancesList"),
                $appliancesLast = $("#appliancesList .list-option-container").last(),
                $appliancesList = $("#appliancesListTotal"),
                $appliancesListLast = $("#appliancesListTotal .list-option-container").last(),
                array = [],
                textObject = "";
            ...
            for (let i = 0; i < workAdressesArray.length; i++) {
                ...
                workSum += price;
                appendObject($work, returnObject(parseData("F" + workAdressesArray[i]), "", spacedNum(Math.round(price)) + " грн."));
            }
            appendObject($work, returnObject(parseData("F" + 66), "", Math.round(workSum * 0.022 * parseData("S42")) + " грн."));
            workSum += workSum * 0.022 * parseData("S42");

            appendObject($work, returnObject(parseData("F" + 67), "", Math.round(((months * 2 * 1200) + 3000 + (space * 100)) * parseData("S42")) + " грн."));
            workSum += ((months * 2 * 1200) + 3000 + (space * 100)) * parseData("S42");

            appendContainer($work, true);
            appendObject($workLast, getHeader("Комплектуючі та чистові матеріали", ""));
            ...
            for (let i = 0; i < materialsAdressesArray.length; i++) {
                ...
                workSum += price;
                appendObject($work, returnObject(parseData("F" + materialsAdressesArray[i]), "", spacedNum(Math.round(price)) + " грн."));
            }

            appendContainer($work, true);
            appendObject($workLast, getHeader("Витрати компанії", "Кількість"));

            appendObject($work, returnObject(parseData("F100"), months + " міс.", ""));
            ...
            for (let i = 0; i < casualtiesAdressesArray.length; i++) {
                ...
                workSum += price;
                appendObject($work, returnObject(parseData("F" + casualtiesAdressesArray[i]), Math.round(price / months) + " грн. / місяць", spacedNum(Math.round(price)) + " грн."));
            }
            workSum += parseFloat(hrnCourse) * parseInt(space) * parseInt(parseData("G37"));
            appendObject($work, returnObject(parseData("F212"), "", Math.round(parseFloat(hrnCourse) * space * parseInt(parseData("G37"))) + " грн."));


            if (furnitureBool) {
                appendBlock($furniture, false, $furnitureLast, "Кухня", "Кількість");
                ...
                appendBlock($furniture, true, $furnitureLast, "Вітальня", "Кількість");
                ...
                appendBlock($furniture, true, $furnitureLast, "Спальня", "Кількість");
                ...
                appendBlock($furniture, true, $furnitureLast, "Світильники", "Кількість");
                ...
                appendBlock($furniture, true, $furnitureLast, "Декор", "Кількість");
                ...
                appendObject($furniture, returnObject(parseData("F" + 162), Math.round(furnitureSum * 0.03 * furnitureRate) + "грн."));
                furnitureSum += (furnitureSum * 0.03 * furnitureRate);
                appendObject($furniture, "<div class=\"division-block pricelist\"></div><div class=\"list-option-container summary\"></div>");
                appendObject($furnitureLast, `<span class=\'name summary\'>Всього по меблях:</span><span class=\'list-text summary work\'>${spacedNum(Math.round(furnitureSum))} грн.</span>`);
            }
            ...
            if (!!parseInt(hygienicShower) || !!parseInt(secondGypsumLayer) || !!parseInt(floorScreed) || !!parseInt(heatedFlooring) || !!parseInt(denoising) || !!parseInt(entranceDoors) || !!parseInt(conditioning)) {
                appendBlock($work, true, $workLast, "Опції", "");
            }
            ...
            if (!furnitureBool) {
                $furniture.toggle(false);
            }

            appendObject($work, "<div class=\"division-block pricelist\"></div><div class=\"list-option-container summary\"></div>");
            appendObject($workLast, `<span class=\'name summary\'>Всього по будівельній частині:</span><span class=\'list-text summary work\'>${spacedNum(Math.round(workSum))} грн.</span>`);
            ...
            function appendFurnitureOption(name, manufacturer, amount, price, dim) {
                ...
                if (!manufacturer) {
                    appendObject($furnitureLast, `<span class=\'name\'>${name}</span><span class=\'list-text\'>${amount} ${dim} </span>`);
                    return;
                }
                appendObject($furnitureLast, `<span class=\'name\'>${name}, ${manufacturer}</span><span class=\'list-text amount\'>${amount} ${dim}</span><span class=\'list-text\'>${spacedNum(Math.round(price * amount * (1 + parseFloat(parseData("S164")/100))))} грн.</span>`);
            }
            appendObject($materials, "<div class=\"division-block pricelist\"></div><div class=\"list-option-container summary\"></div>");
            appendObject($materialsLast, `<span class=\'name summary\'>Всього по будівельній частині:</span><span class=\'list-text summary work\'>${Math.round(workSum)} грн.</span>`);
            let sum = 0;
            ...
            if (appliances !== "undefined") {
                ...
                for (let i = 0; i < array[1]; i++) {
                    appendObject($appliances, "<div class=\"option-block\"><div class=\"division-block white\"></div><div class=\"list-option-container appliances\"></div></div>");
                    appendObject($appliancesList, `<span class=\'name white\'>${parseData("F" + (array[0] + i))} ${parseData("E" + (array[0] + i))}</span><span class=\'list-text white\'>${spacedNum(parseData("D"+ (array[0]+i)))} грн.</span>`);
                    if (!!appliancesBoolTotal) {
                        appendObject($appliancesList, "<div class=\"option-block\"><div class=\"division-block pricelist\"></div><div class=\"list-option-container\"></div></div>");
                        appendObject($appliancesListLast, `<span class=\'name\'>${parseData("F" + (array[0] + i))} ${parseData("E" + (array[0] + i))}</span><span class=\'list-text amount\'>1 шт.</span><span class=\'list-text\'>${spacedNum(parseData("D" + (array[0] + i)))} грн.</span>`);
                    }
                ...
                if (appliancesBoolTotal) {
                    sum += parseFloat(parseData("G36"));
                    appendObject($appliancesList, "<div class=\"option-block\"><div class=\"division-block pricelist\"></div><div class=\"list-option-container\"></div></div>");
                    appendObject($appliancesListLast, `<span class=\'name\'>Доставка техніки</span><span class=\'list-text amount\'></span><span class=\'list-text\'>${parseFloat(quantity) * parseFloat(parseData("G36"))} грн.</span>`);
                    appendObject($appliances, "<div class=\"option-block\"><div class=\"division-block white\"></div><div class=\"list-option-container appliances\"></div></div>");
                    appendObject($appliancesListLast, `<span class = \'name white\'>Доставка техніки</span><span class=\'list-text white\'>${parseFloat(quantity) * parseFloat(parseData("G36"))} грн.</span>`);
                    appendObject($appliancesList, "<div class = \"division-block pricelist\"></div><div class=\"list-option-container summary\"></div>");
                    $appliancesList.html(spacedNum(sum));
                    $("#appliancesTotalDiscount").html(parseFloat(spacedNum(sum * 0.9)));
                    appendObject($appliancesListLast, `<span class=\'name summary\'>Всього по техніці:</span><span class=\'list-text summary work\'>${spacedNum(sum)} грн.</span> <span class=\'name summary\'><b>Всього по техніці, зі знижкою</b>:</span><span class=\'list-text summary work\'>${spacedNum(Math.round(sum * 0.9))} грн.</span>`);
                }
            }
            if (!appliancesBoolTotal) {
            ...
            function getHeader(name, amount) {
                return `<h4 class=\"pricelist-header small no-padding\">${name}</h4><span class=\'notation amount\'>${amount}</span><span class=\'notation\'>Ціна</span>`
            }

            function appendContainer(obj, margined) {
                margined ? appendObject(obj, "</div><div class=\"list-option-container margined\"></div>") : appendObject(obj, "</div><div class=\"list-option-container\"></div>");
            }

            function appendBlock(obj1, margin, obj2, name, amount) {
                appendContainer(obj1, margin);
                appendObject(obj2, getHeader(name, amount));
            }
