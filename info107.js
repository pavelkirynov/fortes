...
const onDataLoaded = (data) => {
    ...
    let appliancesBoolTotal = +cookies._appliancesBoolTotal,
        ...
        appendWorkOption(parseData("F66"), 0, 1, ((workSum - (parseFloat(parseData(`${letter+42}`, space)) + parseFloat(parseData(`${letter+42}`, space)) + parseFloat(parseData(`${letter+43}`, space)) + parseFloat(parseData(`${letter+44}`, space)) * parseFloat(amountOfBathrooms) + parseFloat(parseData(`${letter+45}`, space))) * space)/100)*2.2);
