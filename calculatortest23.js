...
function handleFurniture(furniture, FurnitureMarkup) {
    ...
    if (furnitureBool) {
        ...
        lightingTotal = (element.spotlight * (0.48 * space)) + element.chandelier * (amountOfRooms - 1) + element.kitchenWallLight+ element.livingroomFloorLight /* + element.hangingLight*/ ;
        decorationsTotal = element.tulle * amountOfRooms + element.jalousie;
    }
    lightingTotal += element.bedsideLight * 2 + element.kitchenCeilingLight * 2 ;
    let furnitureTotal = (kitchenTotal + livingroomTotal + bedroomTotal + lightingTotal + decorationsTotal) * 1.03 * (1 + (FurnitureMarkup / 100));
    decorationsTotal = (element.curtains * 3) + element.cornice * 3;
...
function handleWork(work, months) {
    ...
    paintingTotal = (element.wallPainting + element.ceiling2 * ceilingBool.ceiling2) * space + element.ceilingPainting * ceilingBool.ceiling3 * space + element.mouldings;
...
function handleMaterials(materials) {
    ...
    if (space < 100) {
        ...
        vinylAmount = parquetAmount = space - (amountOfBathrooms * 9);
        laminateAmount = space - (amountOfBathrooms * 10);
...
function handleOptions(options) {
    ...
    optionsTotal = optionsBool.floorScreed * element.floorScreed * space + ((amountOfBathrooms - 1) * element.hygienicShower) + optionsBool.heatedFlooring * element.heatedFlooring + ((optionsBool.denoising + ceilingBool.ceiling3) * element.gypsumCeilingDenoising * space) + element.wallsDenoising * optionsBool.denoising*space + optionsBool.conditioning * element.conditioningSplit * space + optionsBool.entranceDoors * element.entranceDoor;
    //optionsTotal = optionsBool.floorScreed * element.floorScreed*space + ((optionsBool.hygienicShower - 1)*amountOfBathrooms*space*element.hygienicShower) + optionsBool.heatedFlooring * element.heatedFlooring + (optionsBool.denoising + ceilingBool.ceiling3) * element.gypsumCeilingDenoising * space + ((ceilingBool.ceiling1 + ceilingBool.ceiling2 + optionsBool.denoising)-1) * space;
    //console.log(optionsBool.heatedFlooring);
    //console.log(optionsTotal/(space*28.5));
    return optionsTotal;
}

function handleConditioning(conditioning, ClimaticMarkup) {
    console.log(conditioning.conditioner * 1.05 * (1 + (ClimaticMarkup / 100)));
...
function handleTotal() {
    ...
    let materials = {
        ...
        modern: {
            ...
            parquet: parseFloat(parseData("L94")),
